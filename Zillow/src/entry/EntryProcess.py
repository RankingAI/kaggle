...
if __name__ == '__main__':
    ...
    OutputDir = '%s/feat/FeatureEngineering/1-hold/ensemble/test' % DataDir
    ...
    print('\n==== Tasks for l2 train ...')
    InputDir = '%s/feat/Preprocess' % DataDir
    OutputDir = '%s/feat/FeatureEngineering/1-hold/ensemble/train' % DataDir
    MonthsOfTest = [7, 8, 9]
    fe = FeatureEngineering('%s/SplitData' % InputDir, OutputDir)
    fe.run(tasks, MonthsOfTest)
    del fe
    ...
    print('\n==== Tasks for l1 test ...')
    InputDir = '%s/feat/Preprocess' % DataDir
    OutputDir = '%s/feat/FeatureEngineering/1-hold/single/test' % DataDir
    MonthsOfTest = [10, 11, 12]
    fe = FeatureEngineering('%s/SplitData' % InputDir, OutputDir)
    fe.run(tasks, MonthsOfTest)
    del fe
    ...
    print('\n==== Tasks for l1 train ...')
    InputDir = '%s/feat/Preprocess' % DataDir
    OutputDir = '%s/feat/FeatureEngineering/1-hold/single/train' % DataDir
    MonthsOfTest = [7, 8, 9]
    fe = FeatureEngineering('%s/SplitData' % InputDir, OutputDir)
    fe.run(tasks, MonthsOfTest)
    del fe
    ...
    #     InputDir = '%s/feat/Preprocess' % DataDir
    #     OutputDir = '%s/feat/FeatureEngineering/single/cv/train/%s' % (DataDir, fold)
