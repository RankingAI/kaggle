...
def sanity_check(depth_dir, truth_dir, predict_dir, output_dir):
    ...
    #
    image_fold_df = pd.read_csv('%s/cv_fold_20180902.txt' % predict_dir, header= None)
    image_fold_df.columns = ['img_name', 'fold']
    image_fold_df['fold'] = image_fold_df['fold'].astype(np.int32)
    #
    eval_df = pd.read_csv('%s/eval_20180902.txt' % '/'.join(predict_dir.split('/')[:-1]), header= None)
    ...
    #
    plot_utils.truth_vs_predict_mask(image_df, '%s/demo.jpg' % output_dir)
...
if __name__ == '__main__':
    ...
    # evaluation log
    EvaluateFile = '%s/eval.txt' % args.model_output
