...
class InceptionResNetV2:
    ...
    def get_model_base(self, include_top=True,
                          weights='imagenet',
                          input_tensor=None,
                          input_shape=None,
                          pooling=None,
        ...
        input_shape = img_utils_1._obtain_input_shape(
            input_shape,
            default_size=299,
            min_size=139,
        ...
        x = self.conv2d_bn(img_input, 32, 3, strides=2, padding='same')
        x = self.conv2d_bn(x, 32, 3, padding='same')
        x = self.conv2d_bn(x, 64, 3)
        x = MaxPooling2D(3, strides=2, padding="same")(x)
        x = self.conv2d_bn(x, 80, 1, padding='same')
        x = self.conv2d_bn(x, 192, 3, padding='same')
        x = MaxPooling2D(3, strides=2, padding='same')(x)
        ...
        branch_0 = self.conv2d_bn(x, 384, 3, strides=2, padding='same')
        branch_1 = self.conv2d_bn(x, 256, 1)
        branch_1 = self.conv2d_bn(branch_1, 256, 3)
        branch_1 = self.conv2d_bn(branch_1, 384, 3, strides=2, padding='same')
        ...
        branch_0 = self.conv2d_bn(branch_0, 384, 3, strides=2, padding='same')
        branch_1 = self.conv2d_bn(x, 256, 1)
        branch_1 = self.conv2d_bn(branch_1, 288, 3, strides=2, padding='same')
        branch_2 = self.conv2d_bn(x, 256, 1)
        branch_2 = self.conv2d_bn(branch_2, 288, 3)
        branch_2 = self.conv2d_bn(branch_2, 320, 3, strides=2, padding='same')
        branch_pool = MaxPooling2D(3, strides=2, padding='same')(x)
        branches = [branch_0, branch_1, branch_2, branch_pool]
