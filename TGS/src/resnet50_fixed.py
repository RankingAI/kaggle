...
class ResNet50:
    ...
    def get_model_base(self, include_top=True, weights='imagenet',
        ...
        if weights == 'imagenet':
            ...
            if(phase == 'train'):
                print('load resnet50 model %s' % weights_path)
                model.load_weights(weights_path)
            elif(phase == 'submit'):
                print('no need to load resnet model yet')
            else:
                print('phase not matched...')
                sys.exit(1)
            if K.backend() == 'theano':
